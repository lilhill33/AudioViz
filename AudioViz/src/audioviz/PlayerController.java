...
public class PlayerController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        ChangeListener<Number> sliderListener = (ObservableValue<? extends Number> ov, Number old_val, Number new_val) -> {
            if (mediaPlayer.getStatus().equals(Status.PAUSED)) {
                Double sliderVal = timeSlider.valueProperty().getValue();
                mediaPlayer.seek(Duration.millis(sliderVal));
                
                Duration ct = mediaPlayer.getCurrentTime();
                double ms = Math.rint(ct.toMillis());
